...
- Estructura: Índice + hojas CM + Excepciones
"""
...
# Colores por defecto
COLOR_DEFAULT_PARAM = "FFD9D9D9"  # gris claro para parámetros sin color
...
def generar_hoja_cm_vertical(ws, codigo_cm, elementos, headers_schedule, colores_dict):
    ...
    # Headers base desde schedule (desde val-sql)
    base_headers = [h for h in headers_schedule if h and h not in campos_extra]
    
    # Orden final: headers schedule + campos extra
    parametros_orden = base_headers + campos_extra
    ...
    # Escribir parámetros en columna A (filas)
    for row_idx, param in enumerate(parametros_orden, 1):
        cell = ws.cell(row=row_idx, column=1)
        cell.value = param
        color = colores_dict.get(param, COLOR_DEFAULT_PARAM)
        aplicar_estilo_celda(cell, color, bold=True)
    
    # Escribir valores por elemento (columnas)
    for col_idx, elem in enumerate(elementos, 2):
        ...
        for row_idx, param in enumerate(parametros_orden, 1):
            ...
            if param == "ElementId":
                ...
                # Parámetro normal desde schedule
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Uso: formatear_tablas_excel_v2.py <ruta_json_datos> <ruta_xlsx_salida>")
        sys.exit(1)
