...
# ── Resolver DATA_DIR dinamicamente ──────────────────────────────────────────
# Este script vive en scripts_cpython/ que esta al mismo nivel que data/
# scripts_cpython/ui_comparacion.py  ->  scripts_cpython/  ->  EXT_ROOT
...
PLANILLAS_HEADERS_JSON = os.path.join(
    DATA_DIR_EXT, 'master', 'planillas_headers_order.json'
)

CONFIG_PROYECTO_ACTIVO = os.path.join(
    DATA_DIR_EXT, 'master', 'config_proyecto_activo.json'
)
...
def get_repo_activo_path():
    ...
    Si la ruta guardada es absoluta y valida, la usa directamente.
    Si no existe en disco, reconstruye la ruta canonica en data/proyectos/
    usando el nup_activo del config (proteccion contra rutas legacy).
    """
    cfg  = cargar_json(CONFIG_PROYECTO_ACTIVO, {})
    ruta = (cfg.get("ruta_repositorio_activo") or "").strip()

    if ruta and os.path.exists(ruta):
        log("get_repo_activo_path: ruta directa -> {}".format(ruta))
        return ruta

    # Fallback: construir ruta canonica desde nup_activo
    nup = (cfg.get("nup_activo") or "").strip()
    if nup:
        ruta_canonica = os.path.join(
            PROYECTOS_DIR,
            u'repositorio_datos_{}.json'.format(nup)
        )
        log("get_repo_activo_path: ruta canonica -> {}".format(ruta_canonica))
        return ruta_canonica

    log("get_repo_activo_path: <no definido>")
    return ""
...
def enriquecer_modelo_con_bd(datos_modelo_cm, repo):
    ...
    try:
        if not repo:
            log("enriquecer_modelo_con_bd: repo vacio, se devuelve modelo sin cambios")
        ...
        log("enriquecer_modelo_con_bd: indice BD con {} claves".format(len(indice_bd)))
        ...
        log("enriquecer_modelo_con_bd: modelo enriquecido")
        return nuevo
    except Exception:
...
def construir_excel_por_planilla(filas_csv, codigos_planillas):
    ...
    for row in filas_csv:
        ...
        for idx_h, _ in enumerate(headers):
            val = row_filtrada[idx_h] if idx_h < len(row_filtrada) else ''
            valores.append(val)
...
# ============================================================================
#               COMPARACION EXCEL vs MODELO (POR CELDA)
# ============================================================================
...
def construir_tabla_comparativa(datos_excel_planilla, datos_modelo_cm, codigos_planillas):
    ...
    for codigo_cm, lista_modelo in datos_modelo_cm.items():
        ...
        filas_excel_base, filas_modelo_base, filas_fusionadas = _comparar_por_planilla(
            codigo_cm, headers, [], lista_modelo
        )
...
def exportar_json_y_formatear(datos_comparacion, data_dir,
    ...
    try:
        ...
        for nombre_planilla, bloque in datos_comparacion.items():
            ...
            datos_por_tabla[codigo_cm] = {'headers': headers, 'filas': filas_fusionadas}
        ...
        messagebox.showerror("Error", "Error en formatear_tablas_excel.py:\n{}".format(e))
        raise
    except Exception:
...
if __name__ == '__main__':
    try:
        ...
        log("Argumentos: script_json={} csv_codigos={} data_dir={} "
            "formatear_script={} ruta_xlsx_salida={} python_exe={} "
            "modelo_json={} headers_json_old={}".format(
