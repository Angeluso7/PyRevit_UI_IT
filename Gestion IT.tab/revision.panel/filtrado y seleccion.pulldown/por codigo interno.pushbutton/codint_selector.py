# -*- coding: utf-8 -*-
"""
Selector de CodIntBIM y modo de aislamiento.
Uso:
    codint_selector.py <IN_JSON> <OUT_JSON>
"""
...
class CodIntSelectorApp:
    def __init__(self, root, datos, out_path):
        ...
        self.codint_unicos = sorted(
            {e.get("codintbim", "") for e in self.datos.get("elementos", []) if e.get("codintbim", "")}
        )
        ...
        sizegrip.place(relx=1.0, rely=1.0, anchor=tk.SE)
    ...
    def _construir_ui(self):
        ...
        rb1 = ttk.Radiobutton(
            frame1, text="Usar filtro por CodIntBIM", variable=self.opcion_var, value="by_codint"
        )
        rb1.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 5))
        ...
        rb2 = ttk.Radiobutton(
            frame2, text="Usar modo Asignados/No asignados", variable=self.opcion_var, value="row2"
        )
        rb2.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 5))

        self.rb_asig = ttk.Radiobutton(
            frame2, text="Asignados", variable=self.asignados_var, value="asignados"
        )
        self.rb_noasig = ttk.Radiobutton(
            frame2, text="No asignados", variable=self.asignados_var, value="no_asignados"
        )
        ...
        btn_cancel = ttk.Button(frame_btn, text="Cancelar", command=self.on_cancelar)
        btn_ok = ttk.Button(frame_btn, text="Aceptar", command=self.on_aceptar)
        btn_cancel.grid(row=0, column=1, sticky="e", padx=(0, 5))
        btn_ok.grid(row=0, column=2, sticky="e")
    ...
    def on_aceptar(self):
        ...
        if op == "by_codint":
            ...
            modo = self.asignados_var.get()
            self._escribir_salida({"opcion": modo})
        self.root.destroy()
...
def main():
    if len(sys.argv) < 3:
        messagebox.showerror("Error", "Uso: codint_selector.py <IN_JSON> <OUT_JSON>")
        return
    ...
    app = CodIntSelectorApp(root, datos, out_path)
    root.mainloop()
