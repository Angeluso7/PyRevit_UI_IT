...
Description:
Selecciona elementos del modelo por CodIntBIM (renglón 1) o
por estado Asignado / No Asignado (renglón 2) usando filtros
c_cod_int / s_cod_int.

En esta versión, el renglón 1 sólo lee CodIntBIM desde el modelo
(host + links) para mostrar en la ventana Tkinter, SIN guardar
ElementId ni CodIntBIM en la BD (repositorio).
...
# -------------------------------------------------------
# Utilidades JSON básicas (sólo para config, no para BD)
# -------------------------------------------------------
...
# -------------------------------------------------------
# Recolección de CodIntBIM DIRECTAMENTE DEL MODELO
# (host + links), sin escribir nada en la BD
# -------------------------------------------------------

def recoger_codintbim():
    """
    Recorre el documento activo y todos los links para
    recoger pares (ElementId, CodIntBIM) y devolverlos
    en un JSON para la ventana Tkinter.
    ...
    # Devolver estructura esperada por codint_selector.py
...
# -------------------------------------------------------
# Helper: obtener Id de parámetro por nombre
# -------------------------------------------------------
...
# -------------------------------------------------------
# Lógica renglón 1 (f_element_x / f_element_y) por CodIntBIM
# -------------------------------------------------------
...
def modificar_filtros_codint(doc_local, nombres_filtros, valor_codint, nombre_parametro="CodIntBIM"):
    ...
    if param_id is None or param_id == ElementId.InvalidElementId:
        forms.alert(
            "No se encontró parámetro '{}' para la regla.".format(nombre_parametro),
...
def main():
    ...
    # Esperar salida del selector Tkinter
    for _ in range(300):
        if os.path.exists(TMP_OUT):
            break
        time.sleep(0.1)
