...
class DatosProyectoApp:
    ...
    def __init__(self, root, data_dir):
        ...
        self.entry_estado = None
        self.entry_decreto = None
        ...
        # Al inicio todo en solo lectura
    ...
    def _construir_ui(self):
        ...
        self.entry_estado = ttk.Entry(main)
        self.entry_estado.grid(row=fila, column=1, columnspan=2, sticky="ew", pady=(0, 5))
        fila += 1

        # 7) Decreto
        ttk.Label(main, text="Decreto:").grid(row=fila, column=0, sticky="w", pady=(0, 5))
        self.entry_decreto = ttk.Entry(main)
        self.entry_decreto.grid(row=fila, column=1, columnspan=2, sticky="ew", pady=(0, 5))
        fila += 1
    ...
    def _cargar_datos_en_campos(self):
        # Refrescar info_activo por si el JSON cambió
        self.registro = cargar_json(self.registro_path, {})
        self.info_activo = self.registro.get(self.nup_activo, {})
        ...
        # Propietario, Estado, Decreto, Comuna
        ...
        self.entry_estado.insert(0, self.info_activo.get("estado", ""))

        self.entry_decreto.config(state="normal")
        self.entry_decreto.delete(0, tk.END)
        self.entry_decreto.insert(0, self.info_activo.get("decreto", ""))
    ...
    def on_aceptar(self):
        ...
        decreto = self.entry_decreto.get().strip()
...
def main():
    if len(sys.argv) < 2:
        messagebox.showerror(
            "Error",
            "Uso: datos_proyecto.py <DATA_DIR>"
        )
        return
