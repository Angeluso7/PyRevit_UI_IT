...
REPO_PATH = os.path.join(DATA_DIR, "repositorio_datos.json")
...
def load_repo():
    try:
        ...
        forms.alert(
            "Error leyendo repositorio_datos.json:\n{}\nSe usará un repositorio vacío.".format(e),
...
def save_repo(data):
    try:
        ...
        forms.alert(
            "Error guardando repositorio_datos.json:\n{}".format(e),
            title="Error repositorio"
        )
...
class ComprobacionForm(Form):
    ...
    def on_resize(self, sender, args):
        # Recolocar botones según tamaño actual
...
def main(repo_tmp_path):
    ...
    # Registros para la ventana de comprobación: [(cod, situación), ...]
    registros_codigos = []
    ...
    for linea in lineas:
        ...
        # Determinar planilla por prefijo
        ...
        if not nombre_planilla:
            # No hay planilla asociada → no se procesa ni se registra
            continue
        ...
        # Buscar elementos en TODOS los documentos por CodIntBIM
        elems_info = find_elements_by_codintbim_all_docs(codintbim)

        # Solo guardar códigos que NO se encuentran en ningún elemento
        if not elems_info:
            registros_codigos.append((codintbim, "Código no encontrado"))
            continue
        
        # Si hay elementos, NO se agrega nada a registros_codigos
    ...
    # Guardar repositorio una sola vez al final
    save_repo(repo)

    # Mensaje resumen simple
    msg = (
        "Combinación completada.\n\n"
        "Filas leídas del Excel: {}\n"
        "Filas que encontraron planilla: {}\n"
        "Elementos afectados (modelo activo + links): {}\n"
    ).format(filas_total, filas_con_planilla, elementos_afectados)

    forms.alert(msg, title="Combinación completada")

    # Mostrar ventana de comprobación de códigos
