...
class ParamsEditor(forms.WPFWindow):
    def __init__(self, linked_doc, element, external_event, handler):
        ...
        self._reemplazos = self._script_data.get("reemplazos_de_nombres", {})
    ...
    def load_parameters_from_repo_or_model(self):
        ...
        try:
            ...
            # 4) aplicar reemplazos de nombres
            reemplazos = self._reemplazos or {}
            parametros_renombrados = {}
            for k, (p_obj, v) in parametros.items():
                nuevo_nombre = reemplazos.get(k, k)
            ...
            for head in headers_order:
                ...
                if head in parametros_renombrados:
                    p, val_model = parametros_renombrados[head]
